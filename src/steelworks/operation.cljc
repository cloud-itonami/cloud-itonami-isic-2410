(ns steelworks.operation
  "OperationActor -- one basic-iron-steel operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Steelworks Advisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Steelworks Manufacturing
  ...
  One graph run = one basic-iron-steel operation (intake ->
  ...
  decision to a human operator (the mill metallurgist). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/dispatch-heat`/`:actuation/issue-
  mill-cert` ALWAYS reach this node when the governor is
  clean -- see `steelworks.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [steelworks.steelworksadvisor :as steelworksadvisor]
...
(defn build
  ...
  `steelworks.store/Store`).
  opts:
    :advisor      -- an `steelworks.steelworksadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (steelworksadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Steelworks Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (steelworksadvisor/-advise advisor store request)]
            {:proposal p :audit [(steelworksadvisor/trace request p)]})))

      ;; Steelworks Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
